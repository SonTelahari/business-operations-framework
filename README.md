...
The shared bridge checks the latest 100 messages in every registered input channel when it starts and every 15 minutes thereafter. A failed startup check is retried while the app deployment settles. Replays are idempotent and bounded by the channel's registration date plus a rolling overlap from the most recent persisted event. This recovers messages missed during a temporary app or database outage without importing Discord history that predates the workspace. Set `BRIDGE_BACKFILL_INTERVAL_SECONDS` to change the periodic interval.
