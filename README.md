...
The shared bridge also checks the latest 100 messages in every registered input channel when it starts. Replays are idempotent and bounded by the channel's registration date plus a rolling overlap from the most recent persisted event. This recovers messages missed during a temporary app or database outage without importing Discord history that predates the workspace.
